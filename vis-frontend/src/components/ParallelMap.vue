<template>
  <div id="parallelMap" style="width: 600px;height:400px;"></div>
</template>
<script>
...
const parallelAxis = [{
        ...
        name: schema[1].text,
        type: 'category',
        data: ['out', 'sign', 'road', 'exhibition', 'mainVenue', 'restroom', 'canteen', 'restroom']
    }, {
        dim: 2,
        name: schema[2].text,
        type: 'category',
...
export default {
    name: 'parallelMap',
    mounted(){
        const myChart = echarts.init(document.getElementById('parallelMap'));
        ...
        this.$bus.$on('daychange', (day) => {
            ...
            this.$axios.get('http://localhost:5270/parallel?day='+day).then(({data}) => {
                console.log(data);
                var dataPTR = [];
                var dataWorker = [];
                var datajournalist = [];
